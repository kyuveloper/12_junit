...
class JupiterAssertionsTests {
    /*
    * junit jupiter는 juni4부터 온 assertion 메소드와 새롭게 자바8 람다 표현식으로 추가된 메소드를 제공한다
    ...
    * */
    ...
    void testAssertEquals(){
        ...
        // 실패시 호출할 메시지
        // Assertions.assertEquals(expected, result, "실패하면 이게 등장");

        // 오차의 허용범위 지정
        // Assertions.assertEquals(expected, result, 1); // 오차 허용 범위 +-1

        // 람다표현식을 이용한 결과는 동일하지만 불필요한 경우 메시지를 만들지 않도록 자연로딩을 이용한다
...
//        );
        ...
        Assertions.assertAll(
                ...
                ()-> Assertions.assertDoesNotThrow(()-> validator.checkDividableNumbers(secondNum, firstNum)), // 오류 유무 확인
    ...
    @Test
    @DisplayName("에상 목록이 실제 목록과 일치하지 않는지 확인")
    void testAssertLinesMatch() {
        ...
        // then
        Assertions.assertLinesMatch(expected, actual, ()-> "두 리스트 값이 일치하지 않음");
    }
}