...
class ParameterizedTests {
    ...
    /*
    * 테스트 시 여러 값들을 검증을 해야하는 모든 경우의 수 만큼 테스트 메소드를 작성해야 한다.
    ...
    * */
    ...
    @ParameterizedTest
    @ValueSource(ints= {1,3,-1,15,123}) // 테스트할 값 지정
    ...
    // @NullSource 와 @EmptySource
    /*
    * 기본 자료형 타입은 Null값을 가질 수 없다.
    * 문자열이나 클래스 타입인 경우 Null이나 빈 값을 파라미터로 전달하기 위해 사용되는 어노테이션이다.
    * null 값과 빈 값을 모두 전달하기 위해 구성된 @NullAndEmptySource도 이용할 수 있다.
    * */
    ...
    * */
    ...
    @ParameterizedTest
    @CsvSource(value = {"test:TEST", "tEst:TEST", "JavaScript:JAVASCRIPT"}, delimiter = ':') // "input : verifyValue"
